...
public sealed class NFSeHttpTransportTests
{
    ...
    public async Task SendAsync_ShouldComposeRequestAgainstConfiguredBaseUrl()
    {
        ...
        using var httpClient = new HttpClient(new CaptureHandler(message =>
        {
            ...
            {
                Content = new StringContent(NFSeLookupXmlFixtures.Success)
                {
                    Headers =
                    {
        ...
        Assert.Equal("https://sefin.producaorestrita.nfse.gov.br/nfse/teste", capturedRequest!.RequestUri?.ToString());
        Assert.Contains(capturedRequest.Headers.Accept, item => item.MediaType == "application/xml");
