...
internal sealed class EmitDpsXmlSchemaValidator
{
    ...
    private static readonly Lazy<XmlSchemaSet> SchemaSet = new(CreateSchemaSet, LazyThreadSafetyMode.ExecutionAndPublication);

    public void Validate(string xmlContent)
    {
        if (string.IsNullOrWhiteSpace(xmlContent))
        {
            throw new NFSeSerializationException("DPS XML content cannot be null or empty.");
        }
        ...
        try
        {
            ...
            if (errors.Count > 0)
            {
                throw new NFSeSerializationException(
                    "DPS XML is not valid against the official NFS-e 1.01 schema. " +
                    string.Join(" ", errors));
            }
        }
        ...
        {
            throw new NFSeSerializationException("DPS XML is malformed.", exception);
        }
        catch (XmlSchemaException exception)
        {
            throw new NFSeSerializationException("Failed to validate DPS XML against the official NFS-e 1.01 schema.", exception);
        }
    }
    ...
    private static XmlSchemaSet CreateSchemaSet()
    {
        ...
        using var stream = resolver.OpenSchema("DPS_v1.01.xsd");
        using var reader = XmlReader.Create(
            ...
            new Uri(BaseSchemaUri, "DPS_v1.01.xsd").ToString());
    ...
    {
        private readonly Assembly _assembly = typeof(EmitDpsXmlSchemaValidator).Assembly;
